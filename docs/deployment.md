...
## 6) (Опционально) Подключение внешнего LLM для генерации ответа (response generation)

В этой реализации:
- Topic classification выполняется локальной NB-моделью (`model_service/model.json`).
- Risk classification выполняется локальными правилами (PII/keywords/policy).
- Внешний OpenAI-compatible LLM (если настроен) используется только для генерации текста ответа/квитанции при авто-завершении safe тикетов.

Для включения генерации через LLM задайте переменные окружения:
- `LLM_BASE_URL` (например, `https://api.openai.com`)
- `LLM_API_KEY`
- `LLM_MODEL`

PII:
- Перед отправкой текста в LLM содержимое тикета маскируется (PII masked).

Если переменные не заданы или LLM недоступен, система автоматически деградирует на локальные шаблонные ответы.
